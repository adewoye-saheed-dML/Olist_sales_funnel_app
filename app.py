...
def clean_data(uploaded_files=None):
    """
    If uploaded_files is None or empty, load from URLs defined in st.secrets
    (or fallback raw GitHub links). Otherwise process the uploaded file objects.
    """
    # 1️⃣ define your auto‑load URLs in secrets.toml or here:
    mql_url   = st.secrets.get("MQL_URL",   "https://raw.githubusercontent.com/adewoye-saheed-dML/Olist_sales_funnel_app/refs/heads/main/olist_marketing_qualified_leads_dataset.csv")
    cld_url   = st.secrets.get("CLD_URL",   "https://raw.githubusercontent.com/adewoye-saheed-dML/Olist_sales_funnel_app/refs/heads/main/olist_closed_deals_dataset.csv")
    order_url = st.secrets.get("ORDER_URL", "https://raw.githubusercontent.com/adewoye-saheed-dML/Olist_sales_funnel_app/refs/heads/main/olist_orders_dataset.csv")

    # if no manual uploads, build a list of URLs
    ...
    for file_or_url in uploaded_files:
        try:
            ...
            st.error(f"Error reading {file_or_url!r}: {e}")
        raise 
